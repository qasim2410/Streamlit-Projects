...
# Load Data (no cache)
# -------------------------------
DATA_URL = r"C:\Users\qasim\OneDrive\Desktop\s\streamlit-sentiment-dashboard - Copy\Tweets.csv"
...
try:
    ...
    if 'tweet_coord' in data.columns:
        coords = data['tweet_coord'].dropna().apply(lambda x: eval(x) if isinstance(x, str) else None)
        coords = coords.dropna()
        if not coords.empty:
            data['lat'] = coords.apply(lambda x: x[0])
            data['lon'] = coords.apply(lambda x: x[1])
except FileNotFoundError:
    st.error(f"❌ File not found: {DATA_URL}")
    st.stop()
...
#     fig = make_subplots(rows=1, cols=len(choice), subplot_titles=choice)
...
if len(choice) > 0:
    ...
    # Pie chart ke liye domain type chahiye
    ...
    for i, airline in enumerate(choice, start=1):
        ...
        if breakdown_type == 'Bar plot':
            fig.add_trace(
                go.Bar(x=df_sent.Sentiment, y=df_sent.Tweets, showlegend=False),
                row=1, col=i
            )
        else:
            fig.add_trace(
                go.Pie(labels=df_sent.Sentiment, values=df_sent.Tweets, showlegend=True),
                row=1, col=i
            )
